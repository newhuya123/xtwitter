...
function sendTalk(noadd) {
    ...
    data: "content="+cont+"&morecontent="+morecontent+"&rank="+GetRandomNum(1,999999),
    success: function(msg){
        if (msg!="error") {
            if (noadd!=1) {
        ...
        success: function(msg){
            if (msg!="您还没有填写要广播的内容！" && msg!="数据传输错误！") {
                if (parseInt(closebox)==1) {
                    ...
                    var firstli,firstli2;
                    firstli=$(".reply_list_ul li").first();
                    if (firstli.length>0) {
                        firstli.before(msg);
                        firstli2=$(".reply_list_ul li").first();
                        firstli2.css("display","none");
                        firstli2.animate({height: 'toggle', opacity: 'toggle'}, { duration: "slow" });
                    } else {
                        $(".reply_list_ul").append(msg);
                        firstli2=$(".reply_list_ul li").first();
                        firstli2.css("display","none");
                        firstli2.animate({height: 'toggle', opacity: 'toggle'}, { duration: "slow" });
                    }
